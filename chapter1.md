...
1 + 1 = ? 
```
우선, 저 명령을 저장할 수 있는 **메모리**와 실행할 수 있는 **연산기**가 필요합니다. 이 메모리를 레지스터(*Register-'저항'을 뜻하는 'resister'와는 다릅니다*)라고 칭하고, 실제 명령을 연산하는 장치의 이름을 산술논리장치(ALU, Arithmetic Logic Unit)이라 합니다. 이 둘을 제어하는 제어부(control unit)이 추가되면, 우리에게 익숙한 부품, CPU(Central Processing Unit)가 완성됩니다.   
...
- 기억장치간 데이터 통신 및 외부 데이터 통신(마우스, 키보드, 모니터, 네트워크 등등)을 관장하며,
- 에러 관리 및 보안 유지 등 시스템 향상성을 유지하려고 합니다.

이를 통해 아래와 같은 구조가 완성되었습니다:
```
+------------------+
|      Kernel      |
| +--------------+ |
...
### 계층 구조
 컴퓨터 시스템은 위에서 설명하다시피, 대개 계층 구조를 채택하고 이를 구현하는 방식으로 이루어져 있습니다. 예시를 위해 가장 대표적인 계층 구조 두가지를 가져와 보았습니다.
